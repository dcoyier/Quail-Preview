...
import { bm25ScoreTerms, embeddingBackendCacheKey, embedTexts, fieldDocumentId, fieldValueToText, loadLocalEmbeddingEnv, loadDatasets, scoreEmbeddingVectorValues, } from "./dataset-store.js";
...
function shouldUsePythonRuntime(code, ctx) {
...
function containsScopedGroupLiteral(code) {
    let quoted;
    for (let i = 0; i < code.length; i++) {
        const ch = code[i];
        if (quoted) {
            if (ch === quoted && code[i - 1] !== "\\")
                quoted = undefined;
            continue;
        }
        if (ch === '"' || ch === "'") {
        ...
        while (next < code.length && /\s/.test(code[next]))
            next++;
        if (/^scope\s*:/i.test(code.slice(next)))
            return true;
    }
    return false;
}
function usesPersistedPythonBinding(code, bindings) {
...
function codeWithoutQuailRawCallBodies(code) {
    ...
    while (i < code.length) {
        ...
        const name = callNames.find((candidate) => isBareQuailCallAt(code, i, candidate));
        if (!name) {
            out += ch;
            i++;
            continue;
...
function isBareQuailCallAt(text, index, name) {
    ...
    return (!/[A-Za-z0-9_]/.test(text[index + name.length] ?? "") &&
        text.slice(index + name.length).trimStart().startsWith("("));
}
function transpileQuailPython(code, ctx) {
...
function extractPythonSourceBindings(code) {
    const bindings = [];
...
function transformQuailStatement(text, knownGroupNames, ctx) {
    ...
    if (assignment) {
        ...
        if (assignmentProducesGroupValue(rhs, { knownGroupNames, ctx }))
            knownGroupNames.add(name);
        else
...
function getKnownGroupVariableNames(ctx) {
    ...
    for (const [name, value] of Object.entries(ctx.variables)) {
        if (valueCanResolveToGroupExpression(value, ctx))
            names.add(name);
    }
    return names;
}
function valueCanResolveToGroupExpression(value, ctx) {
...
function assignableGroupExpressionText(expr, options = {}) {
    const text = normalizeGroupExpressionText(expr);
    if (!text)
        return undefined;
    if (isRawGroupExpressionText(text))
        return text;
...
function assignmentProducesGroupValue(expr, options) {
    const text = normalizeGroupExpressionText(expr);
...
function isComposedGroupExpressionText(text, options) {
    ...
    if (orParts.length > 1)
        return areGroupBooleanParts(orParts, options);
    const andParts = splitTopLevelByWord(normalized, "and");
    if (andParts.length > 1)
        return areGroupBooleanParts(andParts, options);
    return false;
}
function areGroupBooleanParts(parts, options) {
    return parts.some((part) => hasGroupExpressionAnchor(part, options)) &&
        parts.every((part) => isPlausibleGroupExpressionPart(part, options));
}
function isPlausibleGroupExpressionPart(text, options) {
    const normalized = normalizeGroupExpressionText(text);
    if (!normalized || isClearlyScalarExpressionText(normalized))
        return false;
    if (hasDirectGroupExpressionAnchor(normalized, options))
        return true;
    if (normalized.startsWith("not "))
        return isPlausibleGroupExpressionPart(normalized.slice(4), options);
    if (isComposedGroupExpressionText(normalized, options))
        return true;
    if (isListLiteralExpression(normalized))
        return true;
    return /^[A-Za-z_][A-Za-z0-9_]*$/.test(normalized);
}
function hasGroupExpressionAnchor(text, options) {
    const normalized = normalizeGroupExpressionText(text);
    if (hasDirectGroupExpressionAnchor(normalized, options))
        return true;
    if (normalized.startsWith("not "))
        return hasGroupExpressionAnchor(normalized.slice(4), options);
    const orParts = splitTopLevelByWord(normalized, "or");
    if (orParts.length > 1)
        return orParts.some((part) => hasGroupExpressionAnchor(part, options));
    const andParts = splitTopLevelByWord(normalized, "and");
    return andParts.length > 1 && andParts.some((part) => hasGroupExpressionAnchor(part, options));
}
function hasDirectGroupExpressionAnchor(text, options) {
    const normalized = normalizeGroupExpressionText(text);
    if (isRawGroupExpressionText(normalized))
        return true;
    if (isDirectGroupReferenceLiteralText(normalized, options))
        return true;
...
function isDirectGroupReferenceLiteralText(text, options) {
    const normalized = normalizeGroupExpressionText(text);
    if (normalized === "G0" || normalized === "G1" || /^G\d+$/.test(normalized))
        return true;
    return Boolean(options.ctx?.state.groups[normalized]);
}
function isKnownGroupReferenceText(text, options) {
    const normalized = normalizeGroupExpressionText(text);
    if (normalized === "G0" || normalized === "G1" || /^G\d+$/.test(normalized))
        return true;
    if (options.knownGroupNames?.has(normalized))
        return true;
    const ctx = options.ctx;
    if (!ctx)
        return false;
    if (ctx.state.groups[normalized])
        return true;
...
function isClearlyScalarExpressionText(text) {
    const normalized = normalizeGroupExpressionText(text);
...
function transformPythonLiterals(text) {
    return replaceOutsideStrings(replaceOutsideStrings(replaceOutsideStrings(text, /\btrue\b/g, "True"), /\bfalse\b/g, "False"), /\bnull\b/g, "None");
}
function replaceOutsideStrings(text, pattern, replacement) {
    let out = "";
    let start = 0;
    let quoted;
    for (let i = 0; i < text.length; i++) {
...
function transformSpecialPythonCalls(text) {
    const callNames = {
        count: "__quail_count",
    ...
    while (i < text.length) {
        ...
        const name = Object.keys(callNames).find((candidate) => isBareQuailCallAt(text, i, candidate));
        if (!name) {
            out += ch;
            i++;
            continue;
...
function buildPythonRuntimeScript(code, initialVariables, initialBindings, currentBindings) {
...
async function executePythonProgram(code, ctx) {
    ...
    for (const binding of initialBindings)
        sourceByName.set(binding.name, binding);
    ...
    const handleRequest = async (payload) => {
        ...
        try {
            ...
            if (op === "count")
                ...
                value = await saveVariable(arg, ctx, 1);
            else if (op === "count_by")
                value = await countBy(arg, ctx, 1);
            else if (op === "tag") {
    ...
    const processStdoutLine = (line) => {
        ...
        if (line.startsWith(PYTHON_DONE_PREFIX)) {
            completed = true;
            const body = JSON.parse(line.slice(PYTHON_DONE_PREFIX.length));
        ...
        if (line.startsWith(PYTHON_ERROR_PREFIX)) {
            completed = true;
            const body = JSON.parse(line.slice(PYTHON_ERROR_PREFIX.length));
            ctx.errors.push({
    ...
    if (finalSourceBindings) {
        ...
        for (const [name, kind] of Object.entries(finalSourceBindings)) {
            ...
            if (binding)
                nextBindings.push({ ...binding, kind });
        }
    ...
    try {
        ...
        if (assignMatch) {
            ...
            if (assignMatch[2] === "=")
                ...
                ctx.variables[assignMatch[1]] = addValues(current, value);
            else
                ctx.variables[assignMatch[1]] = subtractValues(current, value);
            clearGroupExpressionCache(ctx);
            return;
        }
...
function isRawGroupExpressionText(expr) {
    return /^scope\s*:/i.test(normalizeGroupExpressionText(expr));
}
function normalizeGroupExpressionText(expr) {
...
function normalizeSavedVariableValue(value, line, path) {
    if (value === undefined) {
    ...
    if (typeof value === "number") {
        if (!Number.isFinite(value))
            throw new DslRuntimeError("E_SAVE_VALUE", `Cannot save ${path} because numbers must be finite`, line);
        return value;
    }
...
async function saveVariable(expr, ctx, line) {
    ...
    if (size > maxSize) {
        throw new DslRuntimeError("E_SAVE_VALUE", `Cannot save ${name}; serialized value is ${size} bytes, above the ${maxSize} byte save() limit. Save compact JSON-like summaries only; use g_save(GROUP-EXPR) for groups or recompute large row/id lists within one execution.`, line);
    }
    ctx.variables[name] = value;
    ctx.state.variables[name] = JSON.parse(serialized);
    return value;
}
...
async function executeCreateField(text, ctx, line) {
    const match = text.match(/^create_field\((.+)\)$/);
    if (!match)
        throw new DslRuntimeError("E_PARSE_CREATE_FIELD", "Expected create_field(FIELD)", line);
    const field = await evaluateFieldName(match[1], ctx, line);
    if (!field)
        throw new DslRuntimeError("E_CREATE_FIELD", "create_field requires a non-empty field name", line);
...
async function executeTag(text, ctx, line) {
    ...
    if (!match)
        throw new DslRuntimeError("E_PARSE_TAG", "Expected tag(GROUP-EXPR with FIELD set to TAG)", line);
    ...
    if (values.length === 0)
        throw new DslRuntimeError("E_TAG_VALUE", "tag requires at least one tag value", line);
...
async function resolveTagTargetIds(targetText, ctx, line) {
    ...
    if (Array.isArray(target)) {
        ...
        if (group.scope !== "entries")
            throw new DslRuntimeError("E_TAG_SCOPE", "tag() requires an entry-scoped group expression", line);
...
async function executeUntag(text, ctx, line) {
    ...
    if (removeMatch) {
        ...
        if (values.length === 0)
            throw new DslRuntimeError("E_TAG_VALUE", "untag remove requires at least one tag value", line);
    ...
    if (!match)
        throw new DslRuntimeError("E_PARSE_UNTAG", "Expected untag(FIELD from GROUP-EXPR)", line);
    const field = await evaluateFieldName(match[1], ctx, line);
    const group = await resolveScopedGroupExpression(match[2], ctx, line);
    if (group.scope !== "entries")
        throw new DslRuntimeError("E_UNTAG_SCOPE", "untag() requires an entry-scoped group expression", line);
...
async function evaluateExpression(expr, ctx, line) {
    ...
    if (trimmed.startsWith("retrieve(") && trimmed.endsWith(")"))
        return retrieveUnits(innerCall(trimmed), ctx, line);
    ...
    if (trimmed.startsWith("count(") && trimmed.endsWith(")"))
        return countUnits(innerCall(trimmed), ctx, line);
...
function isRawStringLiteral(text) {
...
function coerceEntryId(value, ctx) {
    const text = coerceId(value);
    if (ctx.entriesById.has(text))
        return text;
    ...
    for (const [id, entryOrdinal] of ctx.entryOrdinalById) {
        if (entryOrdinal === ordinal)
            return id;
    }
    return undefined;
}
function resolveCommandTextArgument(arg, ctx, line) {
...
async function getValue(arg, ctx, line) {
    ...
    if (Array.isArray(evaluated))
        return getEntriesOrFieldValues(evaluated, ctx, line);
...
function getTagFields(ctx) {
    ...
    for (const field of ctx.state.createdFields ?? [])
        fields.add(field);
...
function getEntriesOrFieldValues(values, ctx, line) {
    if (values.length === 0)
        return [];
    ...
    if (strings.every((value) => fields.has(value)))
        return getFieldOrTagValues(strings, ctx);
    throw new DslRuntimeError("E_GET_LIST_SCOPE", "get([...]) requires only entry ids or only field names", line);
}
function formatEntryForGet(entry, ctx) {
    return {
        id: entry.id,
...
function defaultMissingSourceFieldValue(field, dataset) {
    const fieldType = dataset?.manifest.fieldTypes?.[field];
    if (fieldType === "string")
        return "";
    if (dataset?.manifest.embeddedFields?.includes(field) || dataset?.manifest.textFields?.includes(field))
        return "";
    return null;
}
...
function scopedMembers(scope, members, spec) {
    return { scope, members: [...new Set(members)], spec };
}
function inferScopedMembers(values, ctx, spec, line) {
    const members = values.map((value) => coerceString(value));
    if (members.every((member) => ctx.entriesById.has(member)))
        return scopedMembers("entries", members, spec);
    const fields = new Set(getAllFieldNames(ctx));
    if (members.every((member) => fields.has(member)))
        return scopedMembers("fields", members, spec);
    throw new DslRuntimeError("E_GROUP_LIST_SCOPE", "List group expressions must contain only entry ids or only field names", line);
}
function cloneScopedMembers(group) {
    return { ...group, members: [...group.members] };
}
async function resolveScopedGroupExpression(expr, ctx, line) {
    ...
    if (isCacheableGroupExpressionText(text, ctx)) {
        ...
        if (cached)
            return cloneScopedMembers(cached);
...
async function resolveScopedGroupExpressionUncached(text, ctx, line) {
    if (!text || text === "G0")
        return scopedMembers("entries", allEntryIds(ctx), text || "G0");
    if (text === "G1")
        return scopedMembers("fields", getAllFieldNames(ctx), "G1");
    if (isStringLiteral(text)) {
        const value = parseStringLiteral(text, line);
        if (ctx.entriesById.has(value))
            return scopedMembers("entries", [value], text);
        if (getAllFieldNames(ctx).includes(value))
            return scopedMembers("fields", [value], text);
    }
    ...
    if (text in ctx.variables) {
        ...
        if (isGroupExpressionValue(value))
            return resolveScopedGroupExpression(value.expression, ctx, line);
        if (Array.isArray(value))
            return inferScopedMembers(value, ctx, text, line);
        if (typeof value === "string")
            return resolveScopedGroupExpression(value, ctx, line);
...
function combineScopedMembers(parts, op, ctx, line, spec) {
    const [first] = parts;
    if (!first)
        return scopedMembers("entries", [], spec);
    ...
    if (op === "or")
        return scopedMembers(first.scope, parts.flatMap((part) => part.members), spec);
...
function complementScopedMembers(group, ctx, spec) {
...
async function resolveScopedGroup(text, ctx, line) {
    const parts = splitTopLevel(text, ",");
...
async function resolveScopedClauseExpression(scope, expr, ctx, line) {
    const text = trimOuterParens(expr.trim());
    const orParts = splitTopLevelByWord(text, "or");
    if (orParts.length > 1) {
    ...
    if (text.startsWith("scope:")) {
        ...
        if (group.scope !== scope)
            throw new DslRuntimeError("E_GROUP_SCOPE", "Nested scoped clauses must use the same scope", line);
        return group;
    }
    return evaluateGroupClause(scope, text, ctx, line);
}
async function evaluateGroupClause(scope, text, ctx, line) {
    const parenthesizedLeft = splitParenthesizedClauseWithTrailingCondition(text);
    if (parenthesizedLeft)
        return evaluateGroupClause(scope, parenthesizedLeft, ctx, line);
    for (const op of [">=", "<=", "!=", "==", ">", "<"]) {
        ...
        if (optimized)
            return scopedMembers(optimized.scope, optimized.members, text);
    ...
    try {
        ...
        if (!(error instanceof DslRuntimeError) || error.code !== "E_GROUP_EXPR")
            throw error;
    }
    throw new DslRuntimeError("E_GROUP_CLAUSE", `Expected a group clause with a condition, got ${text}`, line);
}
function splitParenthesizedClauseWithTrailingCondition(text) {
    const trimmed = text.trim();
    if (!trimmed.startsWith("("))
        return undefined;
    const closeIndex = findMatchingClose(trimmed, 0, "(", ")");
    if (closeIndex < 0 || closeIndex === trimmed.length - 1)
        return undefined;
...
async function tryImplicitPositiveSimilarityClause(scope, text, ctx, line) {
    if (scope !== "entries")
        return undefined;
    const input = await parseFunctionInput(text, ctx, line);
    if (input.regexOps.length > 0)
        return undefined;
    ...
    if (targets.length !== 1 || targets[0].kind !== "text")
        return undefined;
    const predicate = { field: input.field, text: targets[0].text };
...
async function tryOptimizedScopedGroupClause(scope, leftText, operator, right, ctx, line) {
    if (scope !== "entries" || operator !== ">" || typeof right !== "number" || !Number.isFinite(right))
        return undefined;
...
function matchSimilarityFunction(functionText) {
...
async function evaluateLooseValue(text, ctx, line) {
    const trimmed = text.trim();
    if (isRawStringLiteral(trimmed))
        return parseRegexPatternArg(trimmed, ctx, line);
    try {
        return await evaluateExpression(trimmed, ctx, line);
    }
    catch (error) {
        if (error instanceof DslRuntimeError && error.code === "E_PARSE_EXPR")
            return trimOuterParens(trimmed);
        throw error;
    }
}
function compareClauseValue(left, operator, right) {
    if (Array.isArray(left)) {
        if (operator === "!=")
            return left.every((item) => !compareClauseValue(item, "==", right));
...
async function evaluateClauseValue(scope, member, expression, ctx, line) {
...
async function countUnits(arg, ctx, line) {
    arg = resolveCommandTextArgument(arg, ctx, line);
    ...
    if (!parts)
        return countGroup(arg, ctx, line);
...
async function retrieveUnits(arg, ctx, line) {
    arg = resolveCommandTextArgument(arg, ctx, line);
    if (splitByTopLevelOperator(arg, " out of ")) {
...
function parseRetrieveUnitPrefix(text, line) {
    let rest = text.trim();
    let location = "top";
    const direction = rest.match(/^(top|middle|bottom)\b\s*/i);
    if (direction) {
        location = direction[1].toLowerCase();
    ...
    if (!rest)
        throw new DslRuntimeError("E_PARSE_RETRIEVE", "retrieve requires a unit before of GROUP-EXPR: entries, entries[FIELD], fields, or fields[FIELD]", line);
    return { location, amount, unitText: rest };
}
function selectByLocation(items, location, amount) {
    if (amount <= 0)
        return [];
    if (location === "top")
        return items.slice(0, amount);
    if (location === "bottom")
        return items.slice(-amount).reverse();
...
async function parseUnitSpec(text, ctx, line) {
    const trimmed = text.trim();
...
function buildUnitValuesUncached(unit, group, ctx, line) {
    let values = [];
    let sourceIndex = 0;
    if (unit.kind === "entries") {
        ...
        if (!unit.field || !group.members.includes(unit.field))
            return [];
...
function tagValuesForField(field, ctx) {
    const values = new Map();
    for (const entry of ctx.entries) {
...
function getEntryTagValues(entryId, field, ctx) {
...
function getEntryFunctionValues(entryId, field, ctx) {
    const entry = ctx.entriesById.get(entryId);
    if (!entry)
        return [];
    if (!field)
...
async function parseFunctionInput(expression, ctx, line) {
    let rest = expression.trim();
    let field;
    if (rest.startsWith("[")) {
        const closeIndex = findMatchingClose(rest, 0, "[", "]");
        if (closeIndex < 0)
            throw new DslRuntimeError("E_FIELD_FILTER", `Could not find closing ] in ${expression}`, line);
...
async function evaluateSimilarityTargetsUncached(text, ctx, line) {
...
function similarityTargetExpressionCacheKey(text, ctx) {
    const trimmed = trimOuterParens(text.trim());
    return trimmed;
}
async function tryResolveSimilarityGroupTarget(text, ctx, line) {
    const trimmed = trimOuterParens(text.trim());
    if (!looksLikeSimilarityGroupTarget(trimmed, ctx))
        return undefined;
    return entryGroupSimilarityTarget(await resolveScopedGroupExpression(trimmed, ctx, line), line);
}
function looksLikeSimilarityGroupTarget(text, ctx) {
    if (!text || isStringLiteral(text) || isRawStringLiteral(text))
        return false;
    if (text === "G0" || text === "G1" || /^G\d+$/.test(text))
        return true;
    if (ctx.entriesById.has(text) || ctx.state.groups[text])
        return true;
    if (text.startsWith("scope:"))
        return true;
...
async function similarityTargetFromValue(value, rawText, ctx, line) {
    if (isGroupExpressionValue(value)) {
    ...
    if (scoped)
        return entryGroupSimilarityTarget(scoped, line);
    const entryId = entryIdFromSimilarityValue(value, ctx);
    if (entryId && !isStringLiteral(trimOuterParens(rawText.trim())))
        return entryGroupSimilarityTarget(scopedMembers("entries", [entryId], rawText), line);
...
function scopedMembersFromSimilarityValue(value, rawText, ctx, line) {
    if (!value || typeof value !== "object" || Array.isArray(value))
        return undefined;
    const record = value;
...
function entryIdFromSimilarityValue(value, ctx) {
    if (typeof value === "string" && ctx.entriesById.has(value))
        return value;
    if (value && typeof value === "object" && !Array.isArray(value)) {
        const id = value.id;
        if (typeof id === "string" && ctx.entriesById.has(id))
            return id;
    }
    return undefined;
}
function entryGroupSimilarityTarget(group, line) {
    if (group.scope !== "entries") {
...
function similarityGroupMembersCacheKey(members) {
    return [
        members.length,
        hashStrings(members),
        members[0] ?? "",
...
async function scoreSimilarity(mode, inputText, target, targetAccumulation, context, ctx, line) {
    ...
    if (context.entryId && ctx.entriesById.has(context.entryId)) {
        const predicate = { field: context.field, text: target.text };
        if (mode === "BM25")
            return scoreBm25(context.entryId, predicate, ctx);
...
function lexicalSimilarity(inputText, target) {
...
async function evaluateFieldName(text, ctx, line) {
    const trimmed = text.trim();
    if (isStringLiteral(trimmed))
        return parseStringLiteral(trimmed, line);
    if (trimmed in ctx.variables)
        return coerceString(ctx.variables[trimmed]);
    return trimOuterParens(trimmed);
}
...
async function parseRegexOpsPrefix(text, ctx, line) {
    ...
    while (rest.startsWith(".")) {
        ...
        if (closeIndex < 0)
            throw new DslRuntimeError("E_REGEX", `Could not find closing ) in regex operation ${rest}`, line);
        const arg = rest.slice(openIndex + 1, closeIndex).trim();
        const name = match[1].toLowerCase();
        if (name === "splice") {
            const parts = splitTopLevel(arg, ",");
            if (parts.length !== 2)
                throw new DslRuntimeError("E_REGEX_SPLICE", "splice() requires start and end indices", line);
            ops.push({ type: "splice", start: Number(parts[0].trim()), end: Number(parts[1].trim()) });
        }
        else {
            ops.push({ type: name, pattern: await parseRegexPatternArg(arg, ctx, line) });
        }
        rest = rest.slice(closeIndex + 1).trim();
    }
    return { ops, rest };
}
async function parseRegexPatternArg(text, ctx, line) {
    const trimmed = text.trim();
    if (isRawStringLiteral(trimmed))
        return parseRawStringLiteral(trimmed.slice(1), line);
    if (isStringLiteral(trimmed))
        return parseStringLiteral(trimmed, line);
    try {
...
function parseRawStringLiteral(text, line) {
    if (!isStringLiteral(text))
...
function applyRegexOpsToTexts(texts, ops, line) {
    let out = texts;
    for (const op of ops) {
        if (op.type === "splice") {
...
function compileRegex(pattern, line) {
    if (!pattern)
        throw new DslRuntimeError("E_REGEX", "Regex pattern cannot be empty", line);
    try {
...
function translatePythonInlineRegexFlags(pattern, line) {
    let source = pattern;
...
async function getCachedAsyncScoreVector(key, ctx, compute) {
...
async function getEmbeddingGroupScoreVector(target, field, accumulation, ctx, line) {
    const model = ctx.datasets.find((dataset) => dataset.manifest.embeddingModel)?.manifest.embeddingModel;
    if (!model)
        throw new DslRuntimeError("E_EMBEDDING_MODEL", "No embedding model available for active dataset", line);
    const key = [
...
async function getBm25GroupScoreVector(target, field, accumulation, ctx) {
...
function embeddingVectorsForEntries(entryIds, field, ctx) {
    return entryIds.map((id) => getEntryEmbeddingVector(id, field, ctx));
}
function getEntryEmbeddingVector(id, field, ctx) {
    const dataset = ctx.datasetByEntryId.get(id);
    const entry = ctx.entriesById.get(id);
    if (!dataset || !entry)
        return undefined;
...
function embeddingCentroidForEntries(entryIds, field, accumulation, ctx) {
    let centroid;
    let count = 0;
    for (const id of entryIds) {
        ...
        if (!centroid)
            centroid = new Float32Array(materialized.length);
        const n = Math.min(centroid.length, materialized.length);
        for (let index = 0; index < n; index++)
            centroid[index] += materialized[index] ?? 0;
        count++;
    }
    if (!centroid || count === 0)
...
function materializeEmbeddingVector(vector) {
    const maybeFileBacked = vector;
    return typeof maybeFileBacked.toFloat32Array === "function" ? maybeFileBacked.toFloat32Array() : vector;
}
function groupTargetTexts(target, field, ctx) {
    const texts = [];
