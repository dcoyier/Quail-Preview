...
Content`,
		...
		it("should discover SYSTEM.md from the project Quail config directory", async () => {
			const quailDir = join(cwd, CONFIG_DIR_NAME);
			mkdirSync(quailDir, { recursive: true });
			writeFileSync(join(quailDir, "SYSTEM.md"), "You are a helpful assistant.");
		...
		it("should discover APPEND_SYSTEM.md", async () => {
			const quailDir = join(cwd, CONFIG_DIR_NAME);
			mkdirSync(quailDir, { recursive: true });
			writeFileSync(join(quailDir, "APPEND_SYSTEM.md"), "Additional instructions.");
