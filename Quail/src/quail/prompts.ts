...
export function getDatasetMentionAliases(dataset: DatasetActivationTarget): string[] {
	const baseValues = [dataset.name, dataset.slug];
	const aliases: string[] = [...baseValues];
...
function buildDatasetActivationIndex(datasets: readonly DatasetActivationTarget[]): Array<{ name: string; alias: string; pattern: RegExp }> {
	return datasets
		.flatMap((dataset) =>
			getDatasetMentionAliases(dataset).map((alias) => ({
				name: dataset.name,
				alias,
...
function resolveDatasetMentionName(rawMention: string, datasets: readonly DatasetActivationTarget[]): string | undefined {
...
function getKnownDatasetMentions(text: string, datasets: readonly DatasetActivationTarget[]): string[] {
	const activated = new Set<string>();
	if (datasets.length === 0) return [];

	for (const match of text.matchAll(/@\s*"([^"]+)"/g)) {
		const resolved = resolveDatasetMentionName(match[1], datasets);
		if (resolved) activated.add(resolved);
	}
...
export function getActivatedDatasetNamesFromMessages(
  ...
  datasets: readonly DatasetActivationTarget[] = [],
): string[] {
  ...
  for (const message of messages) {
    ...
    if (datasets.length > 0) {
      for (const name of getKnownDatasetMentions(text, datasets)) active.add(name);
    } else {
...
function loadQuailMainSystemPromptTemplate(): string {
	return readFileSync(
		fileURLToPath(new URL("./field-based-system-prompt.md", import.meta.url)),
		"utf8",
	).trimEnd();
}
...
export function buildQuailMainSystemPrompt(options: {
  ...
  return loadQuailMainSystemPromptTemplate().replace(
    "{{ACTIVE_DATASETS}}",
