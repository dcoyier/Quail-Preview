...
import {
	buildQuailMainSystemPrompt,
	buildQuailProcessingSystemPrompt,
	getActiveDatasetsForPrompt,
	type ActiveDatasetInfo,
} from "../../quail/prompts.js";
import { createQuailQueryToolDefinition } from "../../quail/query-tool.js";
import type { AppDefinition } from "../types.js";
import { quailInteractive } from "./interactive.js";

function buildQuailSystemPromptOverride(options: Parameters<NonNullable<AppDefinition["buildSystemPromptOverride"]>>[0]): string | undefined {
	if (options.customPrompt) {
		return undefined;
	}

	let prompt = buildQuailMainSystemPrompt({
		activeDatasets: options.quailActiveDatasets as ActiveDatasetInfo[] | undefined,
	});
...
export const quailApp: AppDefinition = {
	id: "quail",
	title: "Quail",
	...
	afterAssistantMessage: async ({ cwd, sessionManager, assistantMessage }): Promise<AgentMessage[] | undefined> => {
